# functional-modules-architecture Specification

## Purpose
TBD - created by archiving change create-functional-modules-architecture-skill. Update Purpose after archive.
## Requirements
...
#### Scenario: Missing host PATH tool
- **WHEN** any of the host PATH tools (`python3` ≥ 3.10, `pandoc`, `mmdc`, `textutil` on macOS, or `pdftotext`) is not on PATH at the start of the run
- **THEN** the skill SHALL surface the missing tool to the user immediately, before producing any partial output

#### Scenario: Python dependency installed automatically into venv
- **WHEN** `weasyprint` (or any other Python package listed in `.claude/lib/create-data-dependency-architecture/scripts/python/requirements.txt`) is not installed on the host
- **THEN** the skill SHALL NOT treat this as a missing-tool error; `build-pdf.sh` SHALL bootstrap a virtual environment under `.claude/lib/create-data-dependency-architecture/scripts/python/.venv/`, install the requirements into it, and prepend the venv's `bin/` to `PATH` for the build — host-wide installation is not required and SHALL NOT be requested
