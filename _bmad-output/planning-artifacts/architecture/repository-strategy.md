...
| **`ram-reference-data`** | 0 | Owns all 32 reference-data tables — tier (a) upstream-sourced (15 `jo_*` + `mrd_*`) and tier (b) RAM-owned (`ram_regions` / `ram_offices` / `ram_calendar_periods` + vocabularies), plus `ram_sync_status`. | In-process JOH eLinks nightly sync + MRD weekly blob pick-up (tier a); versioned read API (jurisdiction-filtered[^d8]); tier-(b) maintenance by DBAs via SQL in MVP[^d10]; reads happen via direct SQL by other services. |
