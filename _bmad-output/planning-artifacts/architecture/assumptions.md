...
| **A29** *(reconciled 2026-06-17; Flyway → Liquibase 2026-06-18)* | Schema evolution: **Liquibase** (`liquibase-core`). **Liquibase is NOT in the template baseline** — it is a RAM convention added by `ram-scaffold.sh` (the HMCTS demo repo's Database branch uses Flyway; RAM standardises on Liquibase, per AR19/G1.4a). Liquibase owns RAM Pathfinder's own DDL — table creation, column add/drop, grants. **Not** upstream data movement; the JOH eLinks sync and MRD ingestion (see [`../architecture.md`](../architecture.md) → *Upstream reference-data ingestion*; replaced the retracted Phase 0 ETL[^d3]) load tier-(a) data into tables Liquibase has already created. | Tool sourcing corrected 2026-06-17 (was wrongly attributed to the template baseline); ingestion reframing 2026-06-11; Flyway → Liquibase 2026-06-18 |
