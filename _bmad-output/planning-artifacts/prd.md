...
- **ListAssist** — the SSCS judicial-scheduling tool through which SSCS Tribunals schedule judicial sittings today. Replaced by RAM Pathfinder in **wave 1** (Phase 9). (GAPS, the SSCS case-management system, is **retained**, not decommissioned — analogous to the external case-management systems that consume RAM's APIs per D12.)
...
*Operational platform support (OPT Support in the legacy JI system; equivalent in ListAssist) is handled by external HMCTS roles and is not a RAM Pathfinder user role.*

**Problems being solved:**

1. **Both incumbents have replacement drivers.** In Courts, APEX is unsupported with a fixed end-of-life. In Tribunals (SSCS), judicial scheduling runs on ListAssist, which RAM Pathfinder replaces in wave 1. (GAPS, the SSCS case-management system, is retained, not decommissioned.)
...
**Success:** SSCS jurisdiction migrated to RAM Pathfinder in wave 1 with ListAssist replaced for SSCS scheduling (GAPS case management retained); every HMCTS Courts judicial region migrated in waves 2+ with APEX/JI retired; downstream consumers integrating via API; future HMCTS programmes building on RAM Pathfinder's APIs.

### Key characteristics

1. **Greenfield, not strangler.** Neither ListAssist nor APEX supports strangler decomposition. RAM Pathfinder is built end-to-end before any user moves; the legacy system in each jurisdiction runs unchanged for non-migrated users during phased rollout. No dual-write, no event bus, no synchronisation layer.
...
Each role can complete its legacy workflow on RAM Pathfinder without re-training, and faster or no slower than the **cohort's incumbent system** (ListAssist for the SSCS jurisdiction in wave 1; APEX for Courts jurisdictions in waves 2+):
...
| Behavioural parity per domain service | 100% of manual UAT scripts (run by jurisdiction-incumbent-experienced users — ListAssist-experienced for wave 1, APEX-experienced for waves 2+ — comparing RAM Pathfinder vs the incumbent) signed off before that wave's cutover | D5 reframed + FR60 |
...
Page-level NFRs are carried from the APEX baseline (`functional-modules.md` cross-cutting NFRs); RAM Pathfinder must match or exceed each. SSCS wave-1 cutover verifies these thresholds against ListAssist-equivalent operations as part of the SSCS-cohort readiness assessment[^d11].
...
- **NFR32 — Retention**[^d11][^d3]: Data retention per HMCTS retention schedules. **No data is migrated** — historical transactional data remains in the cohort's incumbent system (ListAssist for SSCS, APEX for Courts); RAM Pathfinder retains only data created in RAM Pathfinder from cutover onward.
...
| **GAPS** | The **SSCS case-management system** (legacy), **retained** — RAM Pathfinder does **not** replace GAPS. In wave 1 RAM replaces the SSCS judicial-scheduling tool (**ListAssist**), not GAPS. GAPS is the SSCS analogue of the external case-management systems that consume RAM's APIs (per D12): it stays in place and consumes RAM's APIs[^d11][^d12]. |
...
| **ListAssist** | The SSCS judicial-scheduling tool used in the SSCS Tribunals jurisdiction today; the SSCS scheduling incumbent and the wave-1 UAT parity reference. Replaced by RAM Pathfinder for SSCS in wave 1[^d11]. (GAPS, the SSCS case-management system, is retained.) |
...
| **RAM Pathfinder** | The API-driven greenfield platform for HMCTS judicial scheduling this PRD describes. Replaces ListAssist for SSCS in wave 1 and JI/APEX for Courts in waves 2+. |
