...
revisionNote: 'SCP 2026-06-10 cascade: ETL FR retracted; FR58–FR61 renumbered FR57–FR60 (60 FRs); upstream ingestion mapped to Epic 0.1; two-population identity in Epics 0.1/0.3; jurisdiction dimension throughout; JOH terminology in Phases 1–8 rows; Phase 9+ reframed jurisdiction-first.'
---
...
## Phase 0 (concrete epics 0.1–0.4 — pending SSCS-cohort revalidation)

| FR | Phase 0 epic / coverage | Post-MVP residual? | Notes |
|---|---|---|---|
| FR1 | [Epic 0.1](phase-0/epic-0.1-user-authenticates.md) (sign-in + two-population identity resolution) + [Epic 0.3](phase-0/epic-0.3-user-populations-bootstrapped.md) (the data resolved against) | — | IdP email → `jo_people` personnel number (JOH) or `ram_auth_staff_identities` UUID (staff),[^d9] |
| FR2 | [Epic 0.1](phase-0/epic-0.1-user-authenticates.md) Story 0.1.5 | — | Authorisation principal → roles + **jurisdiction** + Region/Area scope (read-only API) |
| FR3 | [Epic 0.1](phase-0/epic-0.1-user-authenticates.md) Story 0.1.5 | — | `GET /v1/users/{id}/effective-permissions` |
| **FR4** | **Data layer only** in [Epic 0.3](phase-0/epic-0.3-user-populations-bootstrapped.md) (auth tables maintainable by DBAs via SQL per the identity-bootstrap runbook) | **YES — UI surface** for sysadmins **is post-MVP**[^d10] | Bootstrap mechanism itself is outside the PRD's scope[^d9] |
| FR5 | — | — | Post-MVP per PRD v2.5 (intentional deferral; pre-existing) |
| **FR6** | **Tier (a)**: [Epic 0.1](phase-0/epic-0.1-user-authenticates.md) Stories 0.1.3/0.1.4 (eLinks sync + MRD ingestion — read-only in RAM, corrections at source). **Tier (b)**: [Epic 0.2](phase-0/epic-0.2-reference-data-read-only.md) Story 0.2.1 (tables + seed + DBA runbook). Read API over both tiers: Story 0.2.2. | **YES — tier-(b) RSU maintenance UI** is post-MVP[^d10]. Tier (a) never gets a RAM write surface in any phase. | Two ownership tiers in separate tables preserve lineage |
| FR7 | [Epic 0.1](phase-0/epic-0.1-user-authenticates.md) (tier-(a) grants) + [Epic 0.2](phase-0/epic-0.2-reference-data-read-only.md) (tier-(b) grants + pattern complete) | — | Direct SQL via SELECT grants; **writes follow the tier** (ingestion for tier (a); DBA SQL for tier (b)) |
| FR8 | distributed (lands in [Epic 0.1](phase-0/epic-0.1-user-authenticates.md) first) | — | Shared `ram_configuration_values` Flyway baseline |
...
| FR55 | [Epic 0.1](phase-0/epic-0.1-user-authenticates.md) Story 0.1.7 | — | Home shell with role-scoped navigation |
| FR56 | [Epic 0.1](phase-0/epic-0.1-user-authenticates.md) (business stack) | **Partial — admin stack** is post-MVP[^d10] | `ram-ui` delivered in Phase 0; `ram-admin-ui` post-MVP |
| **FR57** *(was FR58)* | [Epic 0.3](phase-0/epic-0.3-user-populations-bootstrapped.md) (initial all-FALSE flag state keyed by (jurisdiction, region)) — cutover orchestration in Phase 9+ (per-wave flip via direct SQL by DBA) | **Partial — activation toggle UI** is post-MVP | Cutover: `UPDATE ram_auth_user_activation_flags SET activated = TRUE WHERE jurisdiction = '…' AND region = '…'` per rollout runbook |
| **FR58** *(was FR59)* | [Epic 0.2](phase-0/epic-0.2-reference-data-read-only.md) Story 0.2.2 (first exercise) + every service story | — | API-as-Product read-side standards |
| **FR59** *(was FR60)* | [Epic 0.1](phase-0/epic-0.1-user-authenticates.md) Story 0.1.1 (first exercise) + every service story | — | Structured logs + correlation IDs |
| *(NFR24)* | [Epic 0.1](phase-0/epic-0.1-user-authenticates.md) Stories 0.1.3/0.1.4 | MRD reader swaps to API post-MVP (when MRD APIs ship) | JOH eLinks + MRD are MVP integrations[^d11] — listed here because the old map carried the retracted ETL FR in this slot |

> There is no data-migration FR: the former Phase 0 ETL was retracted[^d3] and FR58–FR61 became FR57–FR60. Reference data arrives via upstream ingestion (Epic 0.1); user records are bootstrapped outside the PRD's scope[^d9]; tier-(a) corrections happen at source and tier-(b) changes follow the DBA runbook change-trail.
