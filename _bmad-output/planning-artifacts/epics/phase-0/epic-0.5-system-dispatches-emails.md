...
**Given** `ram-notification` is deployed per Story 0.5.1,
...
**And** on a valid request, the endpoint inserts a row in `ram_notification_dispatches` with status `queued` and the user's principal in `created_by_principal`, then returns `202 Accepted` with `{deliveryId, status}`.
