...
- FR15: RAM Pathfinder exposes ticket information per JOH role through `ram-reference-data`'s read API, combining (a) upstream `jo_tickets` (tier (a) — read-only) and (b) RAM-overlay tickets/authorisations in RAM-owned tables keyed by personnel_number (tier (b)); overlay rows editable by admin staff — DBAs via SQL in MVP.
...
- AR46 — **JOH eLinks sync**: an in-process `@Scheduled` task inside `ram-reference-data` pulls the JOH eLinks API **nightly** and full-refresh-upserts the 15 `jo_*` tables. Upserts key on the upstream natural key (`personnel_number` for `jo_people`); rows absent upstream are **marked inactive, never hard-deleted** (protects FKs from domain tables). No new service principal and no new deployable — the task writes the service's own tables in-process; the only credential is the outbound JOH eLinks API credential in Key Vault.
