...
- FR8 *(revised v2.2)*: Cross-service runtime policy values are stored in a shared `ram_configuration_values` infrastructure table, schema-managed by `ram-architecture`'s Liquibase baseline changelog and SELECT-granted to every RAM Pathfinder service DB role. Per-service config uses Spring profiles + `application.yml` + Azure Key Vault. *(Mirror of PRD FR8 — update the canonical PRD text to match the Flyway → Liquibase change.)*
...
- AR19 *(migration tool changed Flyway → Liquibase, 2026-06-18 — Liquibase is the preferred tool; reverses the v1.4 Liquibase→Flyway alignment)* — **Liquibase** per-service for DDL (each service owns the creation of its tables, columns, indexes, grants): changelogs under `src/main/resources/db/changelog/`, master `db.changelog-master.yaml`, formatted-SQL changesets named `NNN-description.sql`. Liquibase **baseline changelog** in `ram-architecture` owns the shared `ram_configuration_values` table.
