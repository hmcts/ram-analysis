...
| 11 | **Behavioural divergence from APEX** | Per D5 (revised), treat APEX as the behavioural reference verified by **manual UAT performed by APEX-experienced users**. For each domain service, the per-phase UAT script names the workflows / edge cases users are expected to compare in APEX vs NJI, with sign-off captured per role per region before the wave gate. There is no automated APEX-comparison harness |
